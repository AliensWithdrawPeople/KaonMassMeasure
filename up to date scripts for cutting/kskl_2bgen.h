...
// Tue Apr  5 09:46:44 2022 by ROOT version 6.24/06
// from TTree tr_ph/Tree with the non-collinear events
// found on file: tr_ph_kskl_2bodygen600k.root
//////////////////////////////////////////////////////////
...
#ifdef kskl_2bgen_cxx
kskl_2bgen::kskl_2bgen(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("tr_ph_kskl_2bodygen600k.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("tr_ph_kskl_2bodygen600k.root");
      }
...
kskl_2bgen::~kskl_2bgen()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t kskl_2bgen::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t kskl_2bgen::LoadTree(Long64_t entry)
{
...
void kskl_2bgen::Init(TTree *tree)
{
...
void kskl_2bgen::Show(Long64_t entry)
{
...
Int_t kskl_2bgen::Cut(Long64_t entry)
{
