...
#ifdef ksklExp_cxx
ksklExp::ksklExp(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("C:/work/Science/BINP/Kaon Mass Measure/tr_ph/scan2018_omphi_tr_ph_fc_e509.5_v8.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("C:/work/Science/BINP/Kaon Mass Measure/tr_ph/scan2018_omphi_tr_ph_fc_e509.5_v8.root");
      }
...
Int_t ksklExp::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ksklExp::LoadTree(Long64_t entry)
{
...
void ksklExp::Show(Long64_t entry)
{
...
Int_t ksklExp::Cut(Long64_t entry)
{
