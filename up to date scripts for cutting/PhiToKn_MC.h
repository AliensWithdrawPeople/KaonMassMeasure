...
// Tue Jul  4 16:40:47 2023 by ROOT version 6.28/04
// from TTree tr_ph_merged/Tree with the non-collinear events
// found on file: ./MC_Kn501.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Int_t           must[10];   //[nmu]
   Int_t           nsim;
   Int_t           simtype[19];   //[nsim]
   Int_t           simorig[19];   //[nsim]
   Float_t         simmom[19];   //[nsim]
   Float_t         simphi[19];   //[nsim]
   Float_t         simtheta[19];   //[nsim]
   Float_t         simvtx[19];   //[nsim]
   Float_t         simvty[19];   //[nsim]
   Float_t         simvtz[19];   //[nsim]
   Int_t           ncorr;
...
#ifdef PhiToKn_MC_cxx
PhiToKn_MC::PhiToKn_MC(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("./MC_Kn501.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("./MC_Kn501.root");
      }
...
PhiToKn_MC::~PhiToKn_MC()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t PhiToKn_MC::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t PhiToKn_MC::LoadTree(Long64_t entry)
{
...
void PhiToKn_MC::Init(TTree *tree)
{
...
void PhiToKn_MC::Show(Long64_t entry)
{
...
Int_t PhiToKn_MC::Cut(Long64_t entry)
{
