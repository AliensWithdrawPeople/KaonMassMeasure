...
int PhiXsec_DrawAll()
{
    ...
    for(int i = 0; i < 8; i++)
    {
        ...
        auto res1 = hist->Fit("pol0", "SQMEL0", "", 420, 465);
        auto res2 = hist->Fit("pol0", "SQMEL0", "", 540, 578);
        TLine left_bkg(420, res1->Parameter(0), 465, res1->Parameter(0));
        TLine right_bkg(540, res2->Parameter(0), 580, res2->Parameter(0));
        left_bkg.SetLineColor(kBlue);
        right_bkg.SetLineColor(kBlue);
        left_bkg.SetLineStyle(2);
