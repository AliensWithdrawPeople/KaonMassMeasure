...
int KsMassEval_All(bool isExp = false)
{
    ...
    const std::vector<double> meanEnergiesSpectrum_vec = {504.683, 507.762, 508.334, 508.896, 509.451, 509.841, 510.263, 510.694, 510.971, 512.297};

    const std::vector<double> meanEnergiesErr = {0.007, 0.007, 0.008, 0.009, 0.004, 0.005, 0.007, 0.009, 0.009, 0.009};
    const std::vector<double> energy_spread = {0.249, 0.272, 0.248, 0.249, 0.264, 0.281, 0.28, 0.257, 0.261, 0.264};
    ...
    const std::vector<double> deltaE_RC_Smeared = {0.115504, 0.0860349, 0.069423, 0.0598971, 0.0769491, 0.118673, 0.197142, 0.346072, 0.477132, 1.56063};
    ...
    {   
        ...
        meanEnergies[energyPoints[i]] = std::make_pair(meanEnergies_vec[i] - deltaE_RC_Smeared[i], meanEnergiesErr[i]); 
    ...
    else
    {
        for(const auto& energyPoint : energyPoints)
        {
            ...
            auto handlerMC = new HandlerMC(fileNameMC, energyPoint, 0.2, meanEnergies[energyPoint].first, false, false, true, sigma_matrix_window_width);
            // auto handlerMC = new HandlerMC(fileNameMC, energyPoint, 0.3, meanEnergiesSpectrum[energyPoint].first, false, false, true, sigma_matrix_window_width);
            ...
            spectrum_mean_corr.push_back(std::make_tuple(energyPoint, mean, meanErr));
